...
-	An understanding of the [Planning for the landing zone deployable architectures](https://cloud.ibm.com/docs/secure-infrastructure-vpc?topic=secure-infrastructure-vpc-plan).

Minimum required permissions
a.	Administrator platform access role
b.	Manager service access role for the cluster in IBM Cloud Kubernetes Service.

In addition, refer to [Deploying OpenShift Data Foundation on VPC clusters](https://cloud.ibm.com/docs/openshift?topic=openshift-deploy-odf-vpc&interface=ui#ocs-storage-vpc) for complete list of prerequisites.
...
For more information about creating VPC, refer to [VPC multi-zone region](https://cloud.ibm.com/docs/vpc?topic=vpc-creating-a-vpc-in-a-different-region&interface=cli).

3.	Create the three VPC OpenShift clusters (each in different regions and VPCs) and set the ``--disable-outbound-traffic-protection`` parameter for each. Refer to [Creating Clusters](https://cloud.ibm.com/docs/openshift?topic=openshift-openshift_odf_rdr_roks&interface=ui#odf-rdr-clusters) for more information.

4.	Enable [Red Hat OpenShift Data Foundation](https://cloud.ibm.com/docs/openshift?topic=openshift-openshift_odf_rdr_roks&interface=ui#odf-rdr-enable-redhat) on the primary and secondary clusters.

5.	Setup and configure [Red Hat Advanced Cluster Management](https://cloud.ibm.com/docs/openshift?topic=openshift-openshift_odf_rdr_roks&interface=ui#odf-rdr-install-acm) on the ACM cluster.
...
7.	Test the sample [application failover](https://cloud.ibm.com/docs/openshift?topic=openshift-openshift_odf_rdr_roks&interface=ui#odf-rdr-test) from primary to secondary regions and relocate back to primary region.
