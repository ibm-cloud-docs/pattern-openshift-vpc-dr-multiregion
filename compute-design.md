...
The multi-region disaster recovery pattern for Red Hat OpenShift includes compute options that are highly available, properly isolated, provide capacity for the application, software defined storage and backups, and can handle increased workload demands.
...
Refer to the [Red Hat OpenShift on VPC resiliency](/docs/pattern-openshift-vpc-mz-resiliency?topic=pattern-openshift-vpc-mz-resiliency-overview) pattern for additional details on compute design considerations.
