...
| ODF Architecture  | Provides deployment options for OFD | - Local Block Strage \n - Remote Dynamic (VPC) Block Storage \n - Database for etcd | Remote Dynamic (VPC) Block Storage | For lower storage needs or if using Virtual Server instances, VPC Dynamic storage can be a convenient and cost-effective option as the block volumes are provisioned as and when required.
However, for large storage requirements, a bare metal cluster, or high-performance storage with low network latency, local storage would be more suitable.
